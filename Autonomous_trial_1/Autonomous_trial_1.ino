...
float kp=45;                             //45 IF DOESN'T WORK
float kd=0 ;
 ...
 */  
  if(mappedValue!=100)     // add conditions                    
    {
      pid();      
  ...
  else                                 // all white
    {
      /* if(!digitalRead(leftIR)||!digitalRead(rightIR))
        ...
        {
          ACRotate(rotationSpeed);   
          while(mappedValue==100)   //acrotate until finds line   
          {
            sensorMapping();
          }
          pid();
        ...
        else if(right==1)
        {
          ...
          while(mappedValue==100)   //crotate until finds line   
          {
            sensorMapping();
          }      
          pid();
