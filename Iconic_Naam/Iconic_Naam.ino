  ...
  if(mappedValue!=100 || (contiBlack > stopTrigger && allBlack == 1))     // add conditions                    
    {
      ...
      if(insideLoop==1)
      {
        //if(allBlack) loopDirection=1;  //1 is for left turn 0 for right 
        
       if(digitalReading[1]==0 && digitalReading[5]==1 && digitalReading[6]==1&& digitalReading[7]==1)
       {
           CRotate(rotationSpeed); 
           delay(rotateDelay);  
           sensorMapping();
           insideLoop=0;  //apply condition if doesnt work
           loopDirection=0;
       }/*
       if(digitalReading[0]==1 && digitalReading[1]==1 && digitalReading[2]==1&& digitalReading[6]==0 && loopDirection==1)
       {
           ACRotate(rotationSpeed); 
           delay(rotateDelay);  
           sensorMapping();                   //apply condition if doesnt work
           insideLoop=0;
           loopDirection=0;
       }*/
      }
